...
from strategy import SimpleTrendingStrategy
...
res = list(zip(*res))
res_df = pd.DataFrame(list(res[1]), res[0])
res_df.to_csv(os.path.join(output_dir, 'smr_stats_summary.csv'))
###############################################################################
...
for file in files:
    ...
    test_ts = price_ts[split_time:]
    # create strategy
    strat = SimpleTrendingStrategy()
    strat.get_vectorized_signal(test_ts)
    # accept signal and produce position
    port = EqualWeightPort()
    port.get_position(strat.signals.iloc[(n-2):])
    # backtest
    bt = Backtest(test_ts[port.positions.index], port.positions)
    stats = bt.output_summary_stats(periods=1)
    res.append((file_date, stats))
res = list(zip(*res))
res_df = pd.DataFrame(list(res[1]), res[0])
res_df.to_csv(os.path.join(output_dir, 'st_stats_summary.csv'))
###############################################################################
